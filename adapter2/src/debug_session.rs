                ...
                let (var, format) = match expr_format {
                    ...
                    Some(FormatSpec::Array(size)) => {
                        ...
                        let var = if type_class.intersects(TypeClass::Pointer | TypeClass::Reference) {
                            ...
                            let pointee_type = dbg!(var_type.pointee_type());
                            dbg!(pointee_type.is_valid());
                            let array_type = dbg!(pointee_type.array_type(size as u64));
                            dbg!(array_type.is_valid());
                            let addr = sbval.dereference().address().unwrap();
                            dbg!(sbval.target().create_value_from_address("(as array)", &addr, &array_type))
